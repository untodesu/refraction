...
void RayTracingEnvironment::AddQuad( int32 id, const Vector &v1, const Vector &v2, const Vector &v3, const Vector &v4, const Vector &color )
{
...
{
    // far, near, left, right, top, bot faces.
    AddQuad( id, Vector( minc.x, maxc.y, maxc.z ), Vector( maxc.x, maxc.y, maxc.z ), Vector( maxc.x, minc.y, maxc.z ), Vector( minc.x, minc.y, maxc.z ), color );
    ...
    AddQuad( id, Vector( minc.x, maxc.y, maxc.z ), Vector( minc.x, maxc.y, minc.z ), Vector( minc.x, minc.y, minc.z ), Vector( minc.x, minc.y, maxc.z ), color );
    ...
    AddQuad( id, Vector( minc.x, maxc.y, maxc.z ), Vector( maxc.x, maxc.y, maxc.z ), Vector( maxc.x, maxc.y, minc.z ), Vector( minc.x, maxc.y, minc.z ), color );
...
static Vector GetEdgeEquation( Vector p1, Vector p2, int c1, int c2, Vector InsidePoint )
{
    ...
    return Vector( nx / trial_dist, ny / trial_dist, d / trial_dist );
}
        ...
        for( int try_trace = 0; try_trace < 4; try_trace++ ) {
            if( need_trace[try_trace] ) {
                ...
                for( int try2 = try_trace + 1; try2 < 4; try2++ ) {
                    if( need_trace[try2] ) {
                        // Thick conditions!
                        if( SameSign( rays.direction.X( try2 ), rays.direction.X( try_trace ) ) && SameSign( rays.direction.Y( try2 ), rays.direction.Y( try_trace ) ) && SameSign( rays.direction.Z( try2 ), rays.direction.Z( try_trace ) ) ) {
                            need_trace[try2] = 2;
...
void RayTracingEnvironment::Trace4Rays( const FourRays &rays, fltx4 TMin, fltx4 TMax, int DirectionSignMask, RayTracingResult *rslt_out, int32 skip_id, ITransparentTriangleCallback *pCallback )
{
    ...
    for( int c = 0; c < 3; c++ ) {
        fltx4 isect_min_t = MulSIMD( SubSIMD( ReplicateX4( m_MinBound[c] ), rays.origin[c] ), OneOverRayDir[c] );
        fltx4 isect_max_t = MulSIMD( SubSIMD( ReplicateX4( m_MaxBound[c] ), rays.origin[c] ), OneOverRayDir[c] );
    ...
    // based on ray direction, whether to visit left or right node first
        ...
        if( ntris ) {
            ...
            do {
                ...
                if( ( mailboxids[mbox_slot] != tnum ) && ( tri->m_nTriangleID != skip_id ) ) {
                    ...
                    // now, check 3 edges
                    fltx4 hitc1 = AddSIMD( rays.origin[tri->m_nCoordSelect0], MulSIMD( isect_t, rays.direction[tri->m_nCoordSelect0] ) );
                    fltx4 hitc2 = AddSIMD( rays.origin[tri->m_nCoordSelect1], MulSIMD( isect_t, rays.direction[tri->m_nCoordSelect1] ) );

                    // do barycentric coordinate check
                    fltx4 B0 = MulSIMD( ReplicateX4( tri->m_ProjectedEdgeEquations[0] ), hitc1 );
                    B0 = AddSIMD( B0, MulSIMD( ReplicateX4( tri->m_ProjectedEdgeEquations[1] ), hitc2 ) );
                    ...
                    B1 = AddSIMD( B1, MulSIMD( ReplicateX4( tri->m_ProjectedEdgeEquations[4] ), hitc2 ) );
                    ...
                    StoreAlignedSIMD( (float *)rslt_out->HitIds, OrSIMD( AndSIMD( replicated_n, did_hit ), AndNotSIMD( did_hit, LoadAlignedSIMD( (float *)rslt_out->HitIds ) ) ) );
                    rslt_out->HitDistance = OrSIMD( AndSIMD( isect_t, did_hit ), AndNotSIMD( did_hit, rslt_out->HitDistance ) );
...
float RayTracingEnvironment::CalculateCostsOfSplit( int split_plane, int32 const *tri_list, int ntris, Vector MinBound, Vector MaxBound, float &split_value, int &nleft, int &nright, int &nboth )
{
    ...
    if( nleft && !nboth && !nright ) {
        split_value = max_coord;
    }

    if( nright && !nboth && !nleft ) {
...
void RayTracingEnvironment::RefineNode( int node_number, int32 const *tri_list, int ntris, Vector MinBound, Vector MaxBound, int depth )
{
    ...
    // don't try all trinagles as split points when there are a lot of them
        ...
        for( int ts = -1; ts < ntris; ts += tri_skip ) {
            for( int tv = 0; tv < 3; tv++ ) {
                ...
                if( ts == -1 ) {
                    ...
                    // don't try this vertex - not inside
                    if( ( trial_splitvalue > MaxBound[axis] ) || ( trial_splitvalue < MinBound[axis] ) ) {
                        continue;
                    }
                }

                float trial_cost = CalculateCostsOfSplit( axis, tri_list, ntris, MinBound, MaxBound, trial_splitvalue, trial_nleft, trial_nright, trial_nboth );
