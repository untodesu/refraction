...
void RayTracingEnvironment::RenderScene( int width, int height, int stride, uint32 *output_buffer, Vector CameraOrigin, Vector ULCorner, Vector URCorner, Vector LLCorner, Vector LRCorner, RayTraceLightingMode_t lmode )
{
    ...
    for( int y = 0; y < height; y += 2 ) {
        ...
        for( int x = 0; x < width; x += 2 ) {
            ...
            if( ( rslt.HitIds[0] == -1 ) && ( rslt.HitIds[1] == -1 ) && ( rslt.HitIds[2] == -1 ) && ( rslt.HitIds[3] == -1 ) ) {
                ...
                fltx4 bad_dirs = AndSIMD( CmpGtSIMD( ndoti, Four_Zeros ), LoadAlignedSIMD( (float *)signmask ) );
