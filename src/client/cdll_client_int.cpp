...
int GetMaterialIndex( const char *pMaterialName )
{
    if( pMaterialName ) {
        ...
        AssertMsg( nIndex >= 0, "GetMaterialIndex: nIndex >= 0 failed" );
        if( nIndex >= 0 ) {
            return nIndex;
        }
    }
...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
    ...
    modelinfo           = (IVModelInfoClient *)appSystemFactory( VMODELINFO_CLIENT_INTERFACE_VERSION, NULL );
    enginevgui          = (IEngineVGui *)appSystemFactory( VENGINE_VGUI_VERSION, NULL );
    networkstringtable  = (INetworkStringTableContainer *)appSystemFactory( INTERFACENAME_NETWORKSTRINGTABLECLIENT, NULL );
    ...
    inputsystem         = (IInputSystem *)appSystemFactory( INPUTSYSTEM_INTERFACE_VERSION, NULL );
    ...
    if( !engine || !modelrender || !effects || !enginetrace || !render || !debugoverlay || !datacache || !mdlcache || !modelinfo || !enginevgui || !networkstringtable || !partition || !shadowmgr || !staticpropmgr || !enginesound || !filesystem || !gameuifuncs || !gameeventmanager || !soundemitterbase || !inputsystem || !scenefilecache || !gamestatsuploader) {
        return false;
    }
    ...
    ConColorMsg( LOG_COLOR_YELLOW, "%s %u.%u.%u client dll (development build)\n", GAME_NAME, GAME_VERSION_MAJOR, GAME_VERSION_MINOR, GAME_VERSION_PATCH );
#else
    ConColorMsg( LOG_COLOR_YELLOW, "%s %u.%u.%u client dll\n", GAME_NAME, GAME_VERSION_MAJOR, GAME_VERSION_MINOR, GAME_VERSION_PATCH );
#endif
