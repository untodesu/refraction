...
bool R_CullSphere( VPlane const *pPlanes, int nPlanes, const Vector *pCenter, float radius )
{
...
{
    return UseVR() ? STEREO_EYE_LEFT : STEREO_EYE_MONO;
}

StereoEye_t CViewRender::GetLastEye() const
{
    return UseVR() ? STEREO_EYE_RIGHT : STEREO_EYE_MONO;
}
...
void CViewRender::SetUpViews()
{
    ...
    if( UseVR() ) {
        ...
        g_ClientVirtualReality.ProcessCurrentTrackingState( ( flFOVOffset == 0.0f ) ? 0.0f : view.fov );
...
void CViewRender::WriteSaveGameScreenshotOfSize( const char *pFilename, int width, int height, bool bCreatePowerOf2Padded, bool bWriteVTF )
{
...
float ScaleFOVByWidthRatio( float fovDegrees, float ratio )
{
    ...
    float t = tan( halfAngleRadians ) * ratio;
...
void CViewRender::Render( vrect_t *rect )
{
    ...
    for( StereoEye_t eEye = GetFirstEye(); eEye <= GetLastEye(); eEye = (StereoEye_t)( eEye + 1 ) ) {
        ...
        if( ( sv_restrict_aspect_ratio_fov.GetInt() > 0 && engine->IsWindowedMode() && gpGlobals->maxClients > 1 ) || sv_restrict_aspect_ratio_fov.GetInt() == 2 ) {
