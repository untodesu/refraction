...
// hdr parameters
ConVar mat_bloomscale                               ( "mat_bloomscale",                             "0.25"                      );
...
/*
    ...
    src_texture_width,  src_texture_height  -- needed for fixup
*/
static void DrawScreenSpaceRectangleWithSlop( ITexture *dest_rt, IMaterial *pMaterial, int destx, int desty, int width, int height, float src_texture_x0, float src_texture_y0, float src_texture_x1, float src_texture_y1, int src_texture_width, int src_texture_height )
{
    // add slop
    int slopwidth = width + FILTER_KERNEL_SLOP;
    int slopheight = height + FILTER_KERNEL_SLOP;
                ...
                if( !bFirstFrame || !bPerformColCorrect ) {
                    ...
                    if( mat_postprocessing_combine.GetInt() ) {
                        ...
                        if( bPerformSoftwareAA ) {
                            ...
                            // thicc call
                            pRenderContext->DrawScreenSpaceRectangle( aa_mat, partialViewportPostDestRect.x, partialViewportPostDestRect.y, partialViewportPostDestRect.width, partialViewportPostDestRect.height, partialViewportPostSrcCorners.x, partialViewportPostSrcCorners.y, partialViewportPostSrcCorners.z, partialViewportPostSrcCorners.w, dest_rt1->GetActualWidth(), dest_rt1->GetActualHeight(), GetClientWorldEntity()->GetClientRenderable() );
                        ...
                        if( bPerformBloom ) {
                            ...
                            // thicc call
                            pRenderContext->DrawScreenSpaceRectangle( bloom_mat, partialViewportPostDestRect.x, partialViewportPostDestRect.y, partialViewportPostDestRect.width, partialViewportPostDestRect.height, partialViewportPostSrcCorners.x, partialViewportPostSrcCorners.y, partialViewportPostSrcCorners.z, partialViewportPostSrcCorners.w, dest_rt1->GetActualWidth(), dest_rt1->GetActualHeight(), GetClientWorldEntity()->GetClientRenderable() );
