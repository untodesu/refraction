...
public:
    bool Init(IMaterial *pMaterial, KeyValues *pKeyValues)
    {
        if(!CResultProxy::Init(pMaterial, pKeyValues))
            return false;
        m_flFactor = pKeyValues->GetFloat("factor", 1.0f);
        return true;
    }
    ...
    void OnBind(void *pC_BaseEntity)
    {
        ...
        Assert(m_pResult);
        m_pResult->SetFloatValue(pPlayer->GetHealth() * m_flFactor);
...
private:
    CFloatInput m_Factor = CFloatInput();
};
...
public:
    CPlayerLogoProxy()
        : m_Logos(0, 0, LogoLessFunc), m_pDefaultTexture(NULL)
    {
...
protected:
    void OnLogoBindInternal(int playerindex)
    {
        // Find player
        player_info_t info;
