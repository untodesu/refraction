...
bool CServerGameDLL::DLLInit( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CreateInterfaceFn fileSystemFactory, CGlobalVars *pGlobals )
{
    ...
    engine              = (IVEngineServer *)appSystemFactory( INTERFACEVERSION_VENGINESERVER, NULL );
    g_pVoiceServer      = (IVoiceServer *)appSystemFactory( INTERFACEVERSION_VOICESERVER, NULL );
    networkstringtable  = (INetworkStringTableContainer *)appSystemFactory( INTERFACENAME_NETWORKSTRINGTABLESERVER, NULL );
    staticpropmgr       = (IStaticPropMgrServer *)appSystemFactory( INTERFACEVERSION_STATICPROPMGR_SERVER, NULL );
    enginesound         = (IEngineSound *)appSystemFactory( IENGINESOUND_SERVER_INTERFACE_VERSION, NULL );
    partition           = (ISpatialPartition *)appSystemFactory( INTERFACEVERSION_SPATIALPARTITION, NULL );
    modelinfo           = (IVModelInfo *)appSystemFactory( VMODELINFO_SERVER_INTERFACE_VERSION, NULL );
    enginetrace         = (IEngineTrace *)appSystemFactory( INTERFACEVERSION_ENGINETRACE_SERVER, NULL );
    filesystem          = (IFileSystem *)fileSystemFactory( FILESYSTEM_INTERFACE_VERSION, NULL );
    gameeventmanager    = (IGameEventManager2 *)appSystemFactory( INTERFACEVERSION_GAMEEVENTSMANAGER2, NULL );
    datacache           = (IDataCache*)appSystemFactory( DATACACHE_INTERFACE_VERSION, NULL );
    soundemitterbase    = (ISoundEmitterSystemBase *)appSystemFactory( SOUNDEMITTERSYSTEM_INTERFACE_VERSION, NULL );
    gamestatsuploader   = (IUploadGameStats *)appSystemFactory( INTERFACEVERSION_UPLOADGAMESTATS, NULL );
    if( !mdlcache || !engine || !g_pVoiceServer || !networkstringtable || !staticpropmgr || !enginesound || !partition || !modelinfo || !enginetrace || !filesystem || !gameeventmanager || !datacache || !soundemitterbase || !gamestatsuploader ) {
        return false;
    }

    serverpluginhelpers = (IServerPluginHelpers *)appSystemFactory( INTERFACEVERSION_ISERVERPLUGINHELPERS, NULL );
    scenefilecache      = (ISceneFileCache *)appSystemFactory( SCENE_FILE_CACHE_INTERFACE_VERSION, NULL );
    if( !serverpluginhelpers || !scenefilecache ) {
        return false;
    }
...
#ifdef DEV_BUILD
    ConColorMsg( LOG_COLOR_YELLOW, "%s %u.%u.%u server dll (development build)\n", GAME_NAME, GAME_VERSION_MAJOR, GAME_VERSION_MINOR, GAME_VERSION_PATCH );
#else
    ConColorMsg( LOG_COLOR_YELLOW, "%s %u.%u.%u server dll\n", GAME_NAME, GAME_VERSION_MAJOR, GAME_VERSION_MINOR, GAME_VERSION_PATCH );
#endif
...
void CServerGameDLL::GetSaveComment( char *text, int maxlength, float flMinutes, float flSeconds, bool bNoTime )
{
    ...
    if( !bNoTime ) {
        ...
        // Formatted string have 3 digits
        // So if number is greater than 999, it
        // will overflow.
        if( minutes >= 1000 ) {
            minutes = 999;
            seconds = 59;
        ...
        const int minutes_add = ( seconds / 60 );
        seconds %= 60;

        // add the elapsed time at the end of the comment, for the ui to parse out
        Q_snprintf( text, maxlength, "%-64.64s %03d:%02d", map_name, ( minutes + minutes_add ), seconds );
        return;
    }

    // No time!
    Q_snprintf( text, maxlength, "%-64.64s", map_name );
}
...
static bool IsValidPath( const char *pszFilename )
{
    ...
    return !( Q_strlen( pszFilename ) <= 0 || Q_IsAbsolutePath( pszFilename ) || Q_strstr( pszFilename, ".." ) );
}
...
{
    return pEnt ? pEnt->edict() : NULL;
}

CBaseEntity * CServerGameEnts::EdictToBaseEntity( edict_t *pEdict )
{
    return pEdict ? CBaseEntity::Instance( pEdict ) : NULL;
}
...
void CServerGameEnts::CheckTransmit( CCheckTransmitInfo *pInfo, const unsigned short *pEdictIndices, int nEdicts )
{
    ...
    Assert( bIsHLTV == ( pInfo->m_pTransmitAlways != NULL ) || bIsReplay == ( pInfo->m_pTransmitAlways != NULL ) );
...
{
    return g_pGameRules ? g_pGameRules->ClientConnected( pEdict, pszName, pszAddress, reject, maxrejectlen ) : false;
}
