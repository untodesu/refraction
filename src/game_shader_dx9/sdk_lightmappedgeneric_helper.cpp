...
static ConVarRef mat_disable_lightwarp( "mat_disable_lightwarp", true );
static ConVarRef mat_disable_fancy_blending( "mat_disable_fancy_blending", true );
static ConVarRef mat_fullbright( "mat_fullbright", true );
...
void InitParamsLightmappedGeneric( CBaseVSShader *pShader, IMaterialVar **params, const char *pMaterialName, LightmappedGeneric_Vars_t &info )
{
    if( !params[FLASHLIGHTTEXTURE]->IsDefined() ) {
        // FIXME: is there any difference between flashlight and flashlight_border?
    ...
    if( g_pConfig->UseBumpmapping() && params[info.m_nBumpmap]->IsDefined() && params[info.m_nAlbedo]->IsDefined() && params[info.m_nBaseTexture]->IsDefined() && !( params[info.m_nNoDiffuseBumpLighting]->IsDefined() && params[info.m_nNoDiffuseBumpLighting]->GetIntValue() ) ) {
    ...
    if( ( mat_disable_lightwarp.GetBool() ) && ( info.m_nLightWarpTexture != -1 ) ) {
        params[info.m_nLightWarpTexture]->SetUndefined();
    }
    if( ( mat_disable_fancy_blending.GetBool() ) && ( info.m_nBlendModulateTexture != -1 ) ) {
...
void DrawLightmappedGeneric( CBaseVSShader *pShader, IMaterialVar **params, IShaderDynamicAPI *pShaderAPI, IShaderShadow *pShaderShadow, LightmappedGeneric_Vars_t &info, CBasePerMaterialContextData **pContextDataPtr )
{
    ...
    if( pShaderShadow || ( !pContextData ) || pContextData->m_bMaterialVarsChanged || hasFlashlight ) {
        ...
        bool hasParallaxCorrection = params[info.m_nEnvMapOrigin]->IsDefined() && params[info.m_nEnvMapParallax]->IsDefined() && !params[info.m_nEnvMapParallax]->MatrixIsIdentity();
