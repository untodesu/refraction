...
ChunkFileResult_t CMapFile::LoadEntityCallback( CChunkFile *pFile, int nParam )
{
    ...
    if( eResult == ChunkFile_Ok ) {
        ...
        if( !Q_strcmp( "func_detail", pClassName ) ) {
        ...
        if( !Q_strcmp( "func_viscluster", pClassName ) ) {
        ...
        //
        if( !Q_strcmp( "func_ladder", pClassName ) ) {
        ...
        if( !Q_strcmp( "parallax_obb", pClassName ) ) {
            ...
            while( i < pBrush->numsides ) {
                ...
                vCorner = pWinding->p[0];
                vY = pWinding->p[1] - vCorner;
                vZ = pWinding->p[3] - vCorner;
                vX = CrossProduct( vY, vZ ).Normalized();
        ...
        if( !Q_strcmp( "test_sidelist", pClassName ) ) {
            ConvertSideList( mapent, "sides" );
            return ChunkFile_Ok;
        }

        if( !Q_strcmp( "info_overlay", pClassName ) ) {
        ...
        if( !Q_strcmp( "info_overlay_transition", pClassName ) ) {
            // Clear out this entity.
            mapent->epairs = NULL;
            return ( ChunkFile_Ok );
        }
...
{
    if( this != g_MainMap ) {   // all sub-instances will be appended to the main map master list as they are read in
    ...
    for( int i = 0; i < num_entities; i++ ) {
        ...
        if( !Q_strcmp( pEntity, "func_instance" ) ) {
...
void CMapFile::MergeBrushSides( entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin, QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix )
{
    ...
    for( int i = 0; i < Instance->nummapbrushsides; i++ ) {
        ...
        if( !bNeedsTranslation ) {   // check for sides that are part of the world spawn - those need translating
        ...
        if( !bNeedsTranslation ) {   // sides for ladders are outside of the world spawn, but also need translating
        ...
        if( bNeedsTranslation ) {   // we only want to do the adjustment on world spawn brushes, not entity brushes
            if( side->winding ) {
...
void CMapFile::ReplaceInstancePair( epair_t *pPair, entity_t *pInstanceEntity )
{
    ...
    if( !Overwritten && Q_strcmp( pPair->value, NewValue ) != 0 ) {
        free( pPair->value );
...
void CMapFile::MergeEntities( entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin, QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix )
{
    ...
    for( CConnectionPairs *Connection = Instance->m_ConnectionPairs; Connection; Connection = Connection->m_Next ) {
        ...
        if( pos ) {   // null terminate the first field
            *pos = NULL;
